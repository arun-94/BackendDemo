...
public class FloatingActionButtonController {
    ...
    public FloatingActionButtonController(Activity activity, View button) {
        Resources resources = activity.getResources();
        mFabInterpolator = AnimationUtils.loadInterpolator(activity, android.R.interpolator.fast_out_slow_in);
        mFloatingActionButtonWidth = button.getWidth();
        ViewGroup.MarginLayoutParams vlm = (ViewGroup.MarginLayoutParams) button.getLayoutParams();
        mFloatingActionButtonMarginRight = vlm.rightMargin;
        mAnimationDuration = 166;
    ...
    public void onPageScrolled(float positionOffset) {
        ...
        // moves along with it.
        mFloatingActionButton.setTranslationX(
    ...
    public void align(int align, int offsetX, int offsetY, boolean animate) {
        ...
        if (animate && mFloatingActionButton.isShown()) {
            ...
            mFloatingActionButton.setTranslationX(translationX + offsetX);
            mFloatingActionButton.setTranslationY(offsetY);
        }
    }
    ...
    public void resize(int dimension, boolean animate) {
        if (animate) {
            AnimUtils.changeDimensions(mFloatingActionButton, dimension, dimension);
        } else {
            mFloatingActionButton.getLayoutParams().width = dimension;
            mFloatingActionButton.getLayoutParams().height = dimension;
    ...
    public void scaleIn(int delayMs) {
        setVisible(true);
        AnimUtils.scaleIn(mFloatingActionButton, FAB_SCALE_IN_DURATION, delayMs);
        //AnimUtils.fadeIn(mFloatingActionButton, FAB_ICON_FADE_IN_DURATION,
        //       delayMs, null);
    }
    ...
    public void scaleOut() {
        AnimUtils.scaleOut(mFloatingActionButton, mAnimationDuration);
    ...
    private boolean isLayoutRtl() {
        return mFloatingActionButton.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
    }
}